...
class SubLeaderController extends Controller
{
    ...
    {
         $this->middleware('permission:subleader-list|subleader-create|subleader-edit|subleader-delete', ['only' => ['index','store']]);
         $this->middleware('permission:subleader-create', ['only' => ['create','store']]);
         $this->middleware('permission:subleader-edit', ['only' => ['edit','update']]);
         $this->middleware('permission:subleader-delete', ['only' => ['destroy']]);
    }


    public function index()
    {
        return view('backend.sub_leaders.index');
    }

    public function list(Request $request)
    {
        if ($request->ajax()) {
            $data = User::role('subleader')->where('leader_id', Auth::user()->id)->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        $leader->assignRole(['subleader']);

        $notification = [
            'alert-type' => 'success',
            'message' => 'Sub Leader has been added',
        ];
    ...
    public function edit($id)
    {
        ...
        if ($post)
            return view('backend.sub_leaders.edit', compact('post'));
        else
